...
2. **Current pipeline** (`src/lexer/` → `src/compiler/`), wired into `src/main.zig`, which reads a `.stacy` file and runs `compiler.interpret`:
   ...
   - `compiler/instruction.zig`: instructions are fully typed at compile time (`i32.add`, `i8.store x`, `f64.const 0.5` — the arithmetic ops carry a `Type` payload, load/store carry the resolved variable type). The VM never re-derives types.
   - `compiler/vm.zig`: stack VM executing instructions with a variable table; integer arithmetic runs in i64 and narrows to the instruction's type (range-checked); int `/` truncates.
